...
void usage()
{
    cout << "Usage: ./Hybrid-GC-LDPCTest" << endl;
}
...
int main(int argc, char **argv)
{
    ...
    int max_decode_iter_local = stoi(q[0]);
    ...
    if(local_matrix_number == 2)
    {
       ...
       // EncoderFactory *encoder_factory = new EncoderFactory();
        ...
        long long ldpc_fail = 0, error_bit = 0, local_fail = 0;
        long long decode_iter_sum = 0, local_decode_iter_sum = 0, global_decode_iter_sum = 0;
        ...
        for (long long iter = 0; iter < iteration; iter++)
        {
            ...
            // b. disassemble the receive buffer
            for( int i = 0; i < N; i++){
                if( i < N_local){
                    receive_buf_1[i] = receive_buf[i];
                }
                else{
                    receive_buf_2[i - N_local] = receive_buf[i];
                }
            }

            // c. decode information using local matrix
            int decode_iter_local_1 = decoder->decode_local_1(receive_buf_1, corrected_buf_1, encoder->getRowMat1(), encoder->getColMat1(), alpha, beta);
            if(decode_iter_local_max < decode_iter_local_1){decode_iter_local_max = decode_iter_local_1;}
            int decode_iter_local_2 = decoder->decode_local_t(receive_buf_2, corrected_buf_2, encoder->getTRowMat(), encoder->getTColMat(), alpha, beta);
            if(decode_iter_local_max < decode_iter_local_2){decode_iter_local_max = decode_iter_local_2;}

            // d. decode information using global matrix
            int decode_iter_global = 0;   
            if(decode_iter_local_1 == -1 || decode_iter_local_2 == -1){
                local_fail++;
                local_decode_iter_sum += max_decode_iter_local;
                decode_iter_sum += max_decode_iter_local;
                if(decode_iter_local_1 != -1){
                    for(int i = 0; i < N_local; i++){
                        receive_buf[i] = (corrected_buf_1[i] == 0 ? 1 : -1);
                    }
                }
                if(decode_iter_local_2 != -1){
                    for(int i = N_local; i < N; i++){
                        receive_buf[i] = (corrected_buf_2[i - N_local] == 0 ? 1 : -1);
                    }
                }
                decode_iter_global = decoder->decode_global(receive_buf, corrected_buf, encoder->getGlobalRowMat(), encoder->getGlobalColMat(), alpha, beta);
            }
            else{
                local_decode_iter_sum += decode_iter_local_max;
                decode_iter_sum += decode_iter_local_max;
            }

            // e. check whether the result is right and count errors (info + parity)
            if (decode_iter_global == -1)
            {
                global_decode_iter_sum += max_decode_iter_global;
                ...
                global_decode_iter_sum += decode_iter_global;
        ...
        cout << "local decode iter sum = " << local_decode_iter_sum << endl;
        cout << "avg local iter sum = " << (double)local_decode_iter_sum / iteration << endl;
        cout << "global decode iter sum = " << global_decode_iter_sum << endl;
        cout << "avg global iter sum = " << (double)global_decode_iter_sum / local_fail << endl; 
        cout << "decode iter sum = " << decode_iter_sum << endl;
        cout << "avg iter num = " << (double)decode_iter_sum / iteration << endl;
        cout << "local fail number = " << local_fail << endl;
    ...
    // when local_matrix_number = 3
        ...
        for (long long iter = 0; iter < iteration; iter++)
        {
            ...
            for( int i = 0; i < N; i++){
                ...
                else if( i > N_local && i < 2 * N_local){
                    receive_buf_2[i - N_local] = receive_buf[i];
                }
                else if( i > 2 * N_local){
                    receive_buf_3[i - 2 * N_local] = receive_buf[i];
                }
            }
            ...
            int decode_iter_local_1 = decoder->decode_local_1(receive_buf_1, corrected_buf_1, encoder->getRowMat1(), encoder->getColMat1(), alpha, beta);
            if (decode_iter_local_max < decode_iter_local_1)decode_iter_local_max = decode_iter_local_1;
            int decode_iter_local_2 = decoder->decode_local_2(receive_buf_2, corrected_buf_2, encoder->getRowMat2(), encoder->getColMat2(), alpha, beta);
            if (decode_iter_local_max < decode_iter_local_2)decode_iter_local_max = decode_iter_local_2;
            int decode_iter_local_3 = decoder->decode_local_t(receive_buf_3, corrected_buf_3, encoder->getTRowMat(), encoder->getTColMat(), alpha, beta);
            if (decode_iter_local_max < decode_iter_local_3)decode_iter_local_max = decode_iter_local_3;

            // d. decode information using global matrix
            int decode_iter_global = 0;   
            if(decode_iter_local_1 == -1 || decode_iter_local_2 == -1 || decode_iter_local_3 == -1){
                local_fail++;
                local_decode_iter_sum += max_decode_iter_local;
                decode_iter_sum += max_decode_iter_local;
                if(decode_iter_local_1 != -1){
                ...
                if(decode_iter_local_2 != -1){
                    for(int i = N_local; i < 2 * N_local; i++){
                        receive_buf[i] = (corrected_buf_2[i - N_local] == 0 ? 1 : -1);
                    }
                }
                if(decode_iter_local_3 != -1){
                    for(int i = 2 * N_local; i < 3 * N_local; i++){
                        receive_buf[i] = (corrected_buf_3[i - 2 * N_local] == 0 ? 1 : -1);
                    }
                }
                decode_iter_global = decoder->decode_global(receive_buf, corrected_buf, encoder->getGlobalRowMat(), encoder->getGlobalColMat(), alpha, beta);
            }
