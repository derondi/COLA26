...
namespace SSD_Components
{
	...
	void Input_Stream_Manager_SATA::segment_user_request(User_Request* user_request)
	{
		...
		while (handled_sectors_count < req_size) {
			...
			LHA_type lpsa = internal_lsa % host_interface->sectors_per_page; // logical page start address, data in sectors
		    // LHA_type lpea = lpsa + transaction_size - 1; // logical page end address, data in sectors
			...
			if (user_request->Type == UserRequestType::READ) {
				NVM_Transaction_Flash_RD* transaction = new NVM_Transaction_Flash_RD(Transaction_Source_Type::USERIO, SATA_STREAM_ID,
					transaction_size * SECTOR_SIZE_IN_BYTE, lpa, NO_PPA, lpsa, user_request, 0, access_status_bitmap, CurrentTimeStamp);
				...
				NVM_Transaction_Flash_WR* transaction = new NVM_Transaction_Flash_WR(Transaction_Source_Type::USERIO, SATA_STREAM_ID,
					transaction_size * SECTOR_SIZE_IN_BYTE, lpa, lpsa, user_request, 0, access_status_bitmap, CurrentTimeStamp);
				user_request->Transaction_list.push_back(transaction);
	...
	void Host_Interface_SATA::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		val = std::to_string(input_stream_manager->Get_P50_average_read_transaction_turnaround_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Average_P90_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P90_average_read_transaction_turnaround_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Average_P99_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P99_average_read_transaction_turnaround_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);
        */
		attr = "Average_P999_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_turnaround_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);
		...
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_execution_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);
		...
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_transfer_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);
		...
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_waiting_time(SATA_STREAM_ID));
		xmlwriter.Write_attribute_string(attr, val);
