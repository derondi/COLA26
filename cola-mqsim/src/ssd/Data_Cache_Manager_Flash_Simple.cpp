...
{
	Data_Cache_Manager_Flash_Simple::Data_Cache_Manager_Flash_Simple(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, ECCEngine* ecc,
		...
		ecc(ecc), flash_controller(flash_controller), capacity_in_bytes(total_capacity_in_bytes), sector_no_per_page(sector_no_per_page),	request_queue_turn(0), back_pressure_buffer_max_depth(back_pressure_buffer_max_depth)
	{
  ...
  // Only READ transaction could arrive here
	void Data_Cache_Manager_Flash_Simple::handle_transaction_serviced_signal_from_ECC(NVM_Transaction_Flash* transaction)
	{
  ...
  void Data_Cache_Manager_Flash_Simple::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
    ...
    // We will handle this in ECC Engine
    if (transaction->Type == Transaction_Type::READ) {
      return;
    }
