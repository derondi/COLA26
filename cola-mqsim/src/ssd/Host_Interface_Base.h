...
#define DELETE_REQUEST_NVME(REQ) \
    ...
    struct P999_Transaction_Data{
		sim_time_type total_time;      // total time
        sim_time_type execution_time;  // read time
        sim_time_type transfer_time;   // transfer time
        sim_time_type waiting_time;    // wating time


        P999_Transaction_Data() 
        : total_time(0), execution_time(0), transfer_time(0), waiting_time(0) {}
    

        P999_Transaction_Data(double total, double exec, double transfer, double wait)
        : total_time(total), execution_time(exec), transfer_time(transfer), waiting_time(wait) {}
    

        bool operator>(const P999_Transaction_Data& other) const {
	...
	public:
		...
		std::vector<sim_time_type> STAT_P50_transaction_read_times;
		std::vector<sim_time_type> STAT_P90_transaction_read_times;
		std::vector<sim_time_type> STAT_P99_transaction_read_times;
		std::vector<std::pair<sim_time_type, sim_time_type>> STAT_P999_transaction_read_times;
		std::vector<P999_Transaction_Data> STAT_P999_transaction_read_times_A;
	...
	public:
		...
		double Get_P50_average_read_transaction_turnaround_time(stream_id_type stream_id);// in microseconds
		double Get_P90_average_read_transaction_turnaround_time(stream_id_type stream_id);// in microseconds
		double Get_P99_average_read_transaction_turnaround_time(stream_id_type stream_id);// in microseconds
		double Get_P999_average_read_transaction_turnaround_time(stream_id_type stream_id);// in microseconds
		double Get_P999_average_read_transaction_execution_time(stream_id_type stream_id);// in microseconds
		double Get_P999_average_read_transaction_transfer_time(stream_id_type stream_id);// in microseconds
		double Get_P999_average_read_transaction_waiting_time(stream_id_type stream_id);// in microseconds
	...
	public:
		...
		{
      LOG("%s", message->Type == Host_Components::PCIe_Message_Type::READ_COMP ? "read comp" : (message->Type == Host_Components::PCIe_Message_Type::READ_REQ ? "read req": "write req"));
