...
namespace SSD_Components {
  ...
  const ONFI_Channel_NVDDR2 *NVM_PHY_ONFI_NVDDR2::Get_channel(flash_channel_ID_type channelID)
  {
  ...
  ECCMetadata NVM_PHY_ONFI_NVDDR2::GetECCMetadata(flash_channel_ID_type channe_id, flash_chip_ID_type chip_id, flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id, flash_page_ID_type page_id)
  {
    return channels[channe_id]->Chips[chip_id]->GetECCMetadata(die_id, plane_id, block_id, page_id); 
  }
    ...
    int factor = 1;
		...
		* then something illegarl is happening*/
        ...
        factor = dieBKE->ActiveTransactions.front()->decode_type == DECODE_TYPE::HARD ? 3 : 1;
				dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID, factor);
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					...
					for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
						it != transaction_list.end(); it++) {
            uint64_t n_pages = ((*it)->Data_and_metadata_size_in_byte + Flash_Parameter_Set::Page_Capacity - 1) / Flash_Parameter_Set::Page_Capacity;
            uint64_t tDMA = NVDDR2DataInTransferTime((double)n_pages * Flash_Parameter_Set::Page_Capacity * 9 / 8, target_channel);
            
						(*it)->STAT_transfer_time += target_channel->ProgramCommandTime[transaction_list.size()] + tDMA;
						data_transfer_time += tDMA;
					}
					if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
    ...
    int factor = 1;
        ...
        LOG("Read cmd addr transferred, object=%s", ev->Target_sim_object->ID().c_str());
        ...
        factor = dieBKE->ActiveTransactions.front()->decode_type == DECODE_TYPE::HARD ? 3 : 1;
				...
				targetChip->EndCMDXfer(dieBKE->ActiveCommand, factor);
        ...
        if ((NVDDR2_SimEventType)ev->Type == NVDDR2_SimEventType::PROGRAM_CMD_ADDR_DATA_TRANSFERRED) {
          LOG("Program cmd addr data transferred, object=%s", ev->Target_sim_object->ID().c_str());
        } else
          LOG("Program copyback cmd addr data transferred, object=%s", ev->Target_sim_object->ID().c_str());
        ...
        LOG("Read data transferred, object=%s", ev->Target_sim_object->ID().c_str());
				...
				targetChannel->SetStatus(BusChannelStatus::IDLE, targetChip);
				break;
      case NVDDR2_SimEventType::ECC_DATA_TRANSFERRED: {
        ECCEngine::handle_read_data_from_PHY(dieBKE->ActiveTransfer);
    ...
    LOG("data_and_metadata_size_in_byte=%u, channel_width=%u, two_unit_data_out_time=%lu", tr->Data_and_metadata_size_in_byte, channels[tr->Address.ChannelID]->ChannelWidth, channels[tr->Address.ChannelID]->TwoUnitDataOutTime);

    const auto factor = tr->decode_type == DECODE_TYPE::HARD ? 1 : 2;
    //const uint64_t info_per_page = Flash_Parameter_Set::Page_Capacity / (ECC_Parameter_Set::information_bits / 8);

    ECCMetadata metadata = GetECCMetadata(tr->Address.ChannelID, tr->Address.ChipID, tr->Address.DieID,
										  tr->Address.PlaneID, tr->Address.BlockID, tr->Address.PageID);

    const uint64_t ncodeword = tr->nocodeword(metadata.CodewordNumber);
	...
	const uint64_t tDMA_per_codeword = 
	    NVDDR2DataOutTransferTime((double)Flash_Parameter_Set::Page_Capacity / ((8.0 / 9.0) * metadata.CodewordNumber),
                                  channels[tr->Address.ChannelID]) * factor;

	const uint64_t tDMA = ncodeword * tDMA_per_codeword;

	Simulator->Register_sim_event(Simulator->Time() + tDMA_per_codeword,
            this, dieBKE, (int)NVDDR2_SimEventType::ECC_DATA_TRANSFERRED);

	Simulator->Register_sim_event(Simulator->Time() + tDMA,
    ...
    const uint64_t n_info_bits =
        std::max((tr->Data_and_metadata_size_in_byte + ECC_Parameter_Set::information_bits / 8 - 1) /
                     (ECC_Parameter_Set::information_bits / 8),
                 (uint64_t)1);
    const uint64_t tDMA_per_info_bits =
        NVDDR2DataOutTransferTime((double)ECC_Parameter_Set::information_bits / 8 + (double)ECC_Parameter_Set::parity_bits / 8,
                                  channels[tr->Address.ChannelID]) * factor;
    const uint64_t tDMA = n_info_bits * tDMA_per_info_bits;

	Simulator->Register_sim_event(Simulator->Time() + tDMA_per_info_bits,
			this, dieBKE, (int)NVDDR2_SimEventType::ECC_DATA_TRANSFERRED);

	Simulator->Register_sim_event(Simulator->Time() + tDMA,
			this, dieBKE, (int)NVDDR2_SimEventType::READ_DATA_TRANSFERRED);
    */
	tr->STAT_transfer_time += tDMA;
