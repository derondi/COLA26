...
{
	Data_Cache_Manager_Flash_Advanced::Data_Cache_Manager_Flash_Advanced(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, ECCEngine* ecc,
		NVM_PHY_ONFI* flash_controller, unsigned int total_capacity_in_bytes,
		...
		ecc(ecc), flash_controller(flash_controller), capacity_in_bytes(total_capacity_in_bytes), sector_no_per_page(sector_no_per_page),	memory_channel_is_busy(false),
				...
				if (!per_stream_cache[tr->Stream_id]->Check_free_slot_availability()) {
					...
					if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
						evicted_cache_slots->push_back(new NVM_Transaction_Flash_WR(Transaction_Source_Type::CACHE,
							...
							evicted_slot.LPA, tr->LPSA, NULL, IO_Flow_Priority_Class::URGENT, evicted_slot.Content, evicted_slot.State_bitmap_of_existing_sectors, evicted_slot.Timestamp));
  ...
  // Only READ transaction could arrive at here
	void Data_Cache_Manager_Flash_Advanced::handle_transaction_serviced_signal_from_ECC(NVM_Transaction_Flash* transaction)
	{
    LOG();
						...
						if (!((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Check_free_slot_availability()) {
							...
							if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
								...
								evicted_cache_slots->push_back(new NVM_Transaction_Flash_WR(Transaction_Source_Type::USERIO,
									transaction->Stream_id, transfer_info->Size_in_bytes, evicted_slot.LPA, transaction-> LPSA, NULL, IO_Flow_Priority_Class::UNDEFINED, evicted_slot.Content,
  ...
  // Except READ
	...
	void Data_Cache_Manager_Flash_Advanced::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
        ...
        if (eventType == Data_Cache_Simulation_Event_Type::MEMORY_READ_FOR_USERIO_FINISHED)
          LOG("memory read for user io finished");
        else 
          LOG("memory write for user io finished");
        ...
        LOG("memeory read for cache eviction finished");
        ...
        LOG("memory write for cache finished");
