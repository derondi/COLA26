...
namespace SSD_Components
{
	...
	{
		for (unsigned int i = 0; i < _my_instance->chip_no_per_channel; i++) { //! WJY: For every chip in this channel, 
      ...
      bool is_set = false;
			...
			for (Flash_Transaction_Queue::iterator it = sourceQueue1->begin(); it != sourceQueue1->end();)
			{
				if (transaction_is_ready(*it) && (*it)->Address.DieID == dieID && !(planeVector & 1 << (*it)->Address.PlaneID))
				{
					...
					{
            // Check if we need to smart decoding, and make sure that all the transaction inside the command is same type.
            const auto& addr = (*it)->Address;
            const auto metadata = _NVMController->GetECCMetadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID);
            ...
            if (!is_set)
            {
              is_hard_decode = (*it)->decode_type == DECODE_TYPE::HARD;
              is_set = true;
            }

            if (((*it)->decode_type == DECODE_TYPE::HARD) == is_hard_decode)
            {
