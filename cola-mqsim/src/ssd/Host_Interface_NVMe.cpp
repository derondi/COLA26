...
void Input_Stream_Manager_NVMe::segment_user_request(User_Request *user_request)
{
	...
	while (handled_sectors_count < req_size)
	{
		...
		LHA_type lpsa = internal_lsa % host_interface->sectors_per_page; // data in sectors
		//LHA_type lpea = lpsa + transaction_size - 1; // data in sectors
		//printf("lpsa: %lu, lpea: %lu, transaction size: %lu\n", lpsa, lpea, transaction_size);
		...
		if (user_request->Type == UserRequestType::READ)
		{
			NVM_Transaction_Flash_RD *transaction = new NVM_Transaction_Flash_RD(Transaction_Source_Type::USERIO, user_request->Stream_id,
																				 transaction_size * SECTOR_SIZE_IN_BYTE, lpa, NO_PPA, lpsa, user_request, user_request->Priority_class, 0, access_status_bitmap, CurrentTimeStamp);
		...
		{ //user_request->Type == UserRequestType::WRITE
			NVM_Transaction_Flash_WR *transaction = new NVM_Transaction_Flash_WR(Transaction_Source_Type::USERIO, user_request->Stream_id,
																				 transaction_size * SECTOR_SIZE_IN_BYTE, lpa, lpsa, user_request, user_request->Priority_class, 0, access_status_bitmap, CurrentTimeStamp);
			user_request->Transaction_list.push_back(transaction);
...
void Request_Fetch_Unit_NVMe::Process_pcie_write_message(uint64_t address, void *payload, unsigned int payload_size)
{
    ...
    LOG("submission queue register 1");
    ...
    LOG("completion queue register 1");
    ...
    LOG("submission queue register 2");
    ...
    LOG("completion queue register 2");
    ...
    LOG("submission queue register 3");
    ...
    LOG("completion queue register 3");
    ...
    LOG("submission queue register 4");
    ...
    LOG("completion queue register 4");
    ...
    LOG("submission queue register 5");
    ...
    LOG("completion queue register 5");
    ...
    LOG("submission queue register 6");
    ...
    LOG("completion queue register 6");
    ...
    LOG("submission queue register 7");
    ...
    LOG("submission queue register 8");
    ...
    LOG("completion queue register 8");
...
void Request_Fetch_Unit_NVMe::Process_pcie_read_message(uint64_t address, void *payload, unsigned int payload_size)
{
	...
	case DMA_Req_Type::REQUEST_INFO:
	{
      ...
      LOG("NVMe read cmd info, read cmd");
      ...
      LOG("NVMe read cmd info, write cmd");
...
void Host_Interface_NVMe::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter &xmlwriter)
{
	...
	for (unsigned int stream_id = 0; stream_id < no_of_input_streams; stream_id++)
	{
		...
		xmlwriter.Write_attribute_string(attr, val);
        /*
		attr = "Average_P50_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P50_average_read_transaction_turnaround_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Average_P90_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P90_average_read_transaction_turnaround_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Average_P99_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P99_average_read_transaction_turnaround_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);
        */
		attr = "Average_P999_Read_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_turnaround_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Average_P999_Read_Transaction_Execution_Time";
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_execution_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Average_P999_Read_Transaction_Transfer_Time";
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_transfer_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Average_P999_Read_Transaction_Waiting_Time";
		val = std::to_string(input_stream_manager->Get_P999_average_read_transaction_waiting_time(stream_id));
		xmlwriter.Write_attribute_string(attr, val);
