...
namespace SSD_Components
{
	...
	GC_and_WL_Unit_Base::GC_and_WL_Unit_Base(const sim_object_id_type& id,
		Address_Mapping_Unit_Base* address_mapping_unit, Flash_Block_Manager_Base* block_manager, TSU_Base* tsu, ECCEngine* ecc, NVM_PHY_ONFI* flash_controller,
		...
		Sim_Object(id), address_mapping_unit(address_mapping_unit), block_manager(block_manager), tsu(tsu), ecc(ecc), flash_controller(flash_controller), force_gc(false),
    ...
    LOG();
		...
		ecc->ConnectToTransactionServicedSignal(handle_transaction_serviced_signal_from_ECC);
		flash_controller->ConnectToTransactionServicedSignal(handle_transaction_serviced_signal_from_PHY);
	}

  // Only READ transaction could arrive here
	void GC_and_WL_Unit_Base::handle_transaction_serviced_signal_from_ECC(NVM_Transaction_Flash* transaction)
	{
    LOG();
    ...
    if (transaction->Type != Transaction_Type::READ) {
      ...
      return;
			...
			case Transaction_Source_Type::CACHE:
				switch (transaction->Type)
				{
					...
					case Transaction_Type::WRITE:
						...
						break;
					default:
						PRINT_ERROR("ECC Unexpected situation in the GC_and_WL_Unit_Base function!")
				}
				if (_my_instance->block_manager->Block_has_ongoing_gc_wl(transaction->Address)) {
								...
								if (_my_instance->block_manager->Is_page_valid(block, pageID)) {
									...
									if (_my_instance->use_copyback) {
										gc_wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, _my_instance->address_mapping_unit->Convert_address_to_ppa(gc_wl_candidate_address), DEFAULT_LPSA, nullptr, 0, nullptr, 0, INVALID_TIME_STAMP);
										gc_wl_write->ExecutionMode = WriteExecutionModeType::COPYBACK;
										// _my_instance->tsu->Submit_transaction(gc_wl_write);
                    // Write transaction is submitted to ECC Engine for encoding
                    _my_instance->ecc->submit_write_transactions(gc_wl_write);
									} else {
										gc_wl_read = new NVM_Transaction_Flash_RD(Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, _my_instance->address_mapping_unit->Convert_address_to_ppa(gc_wl_candidate_address), DEFAULT_LPSA, gc_wl_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
										gc_wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, NO_PPA, DEFAULT_LPSA, gc_wl_candidate_address, NULL, 0, gc_wl_read, 0, INVALID_TIME_STAMP);
                    ...
                    // Read Transaction is directly submitted to ECC Engine.
		...
		switch (transaction->Type) {
            ...
            // RelatedWrite is a write transaction, so it is firstly submitted to ECC Engine for encoding.
            _my_instance->ecc->submit_write_transactions(((NVM_Transaction_Flash_RD *)transaction)->RelatedWrite);
            _my_instance->ecc->encode();
						_my_instance->tsu->Schedule();
					} else {
  ...
  // Except READ
	...
	{
    LOG();
    // skip READ transaction, we handle this in ECC engine
    if (transaction->Type == Transaction_Type::READ) {
      return;
    }
			...
			case Transaction_Source_Type::CACHE:
				switch (transaction->Type)
				{
					...
					case Transaction_Type::WRITE:
						...
						break;
					default:
						PRINT_ERROR("PHY Unexpected situation in the GC_and_WL_Unit_Base function!")
				}
				if (_my_instance->block_manager->Block_has_ongoing_gc_wl(transaction->Address)) {
					if (_my_instance->block_manager->Can_execute_gc_wl(transaction->Address)) {
						NVM::FlashMemory::Physical_Page_Address gc_wl_candidate_address(transaction->Address);
						...
						if (block->Current_page_write_index - block->Invalid_page_count > 0) {
							...
							for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
								if (_my_instance->block_manager->Is_page_valid(block, pageID)) {
									...
									if (_my_instance->use_copyback) {
										gc_wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, _my_instance->address_mapping_unit->Convert_address_to_ppa(gc_wl_candidate_address), DEFAULT_LPSA, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
										gc_wl_write->ExecutionMode = WriteExecutionModeType::COPYBACK;
										// _my_instance->tsu->Submit_transaction(gc_wl_write);
                    // Write transaction is firstly submitted to ECC Engine for encoding.
                    _my_instance->ecc->submit_write_transactions(gc_wl_write);
									} else {
										gc_wl_read = new NVM_Transaction_Flash_RD(Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, _my_instance->address_mapping_unit->Convert_address_to_ppa(gc_wl_candidate_address), DEFAULT_LPSA, gc_wl_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
			...
			case Transaction_Type::READ:
			{
				...
				if (pbke->Blocks[transaction->Address.BlockID].Holds_mapping_data) {
					...
					if (mppa == transaction->PPA) {
						...
						_my_instance->address_mapping_unit->Allocate_new_page_for_gc(((NVM_Transaction_Flash_RD*)transaction)->RelatedWrite, pbke->Blocks[transaction->Address.BlockID].Holds_mapping_data);
						// _my_instance->tsu->Submit_transaction(((NVM_Transaction_Flash_RD*)transaction)->RelatedWrite);
            // RelatedRead is a read transaction, so it is firstly submitted to ECC Engine for encoding.
            _my_instance->ecc->submit_write_transactions(((NVM_Transaction_Flash_RD *)transaction)->RelatedWrite);
            _my_instance->ecc->encode();
						_my_instance->tsu->Schedule();
					} else {
    ...
    LOG();
					...
					if (block_manager->Is_page_valid(block, pageID)) {
						...
						if (use_copyback) {
							wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								NO_LPA, address_mapping_unit->Convert_address_to_ppa(wl_candidate_address), DEFAULT_LPSA, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
							wl_write->ExecutionMode = WriteExecutionModeType::COPYBACK;
							// tsu->Submit_transaction(wl_write);
              // Write transaction is firstly submitted to ECC Engine for encoding.
              ecc->submit_write_transactions(wl_write);
						} else {
							wl_read = new NVM_Transaction_Flash_RD(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								NO_LPA, address_mapping_unit->Convert_address_to_ppa(wl_candidate_address), DEFAULT_LPSA, wl_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
							wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								NO_LPA, NO_PPA, DEFAULT_LPSA, wl_candidate_address, NULL, 0, wl_read, 0, INVALID_TIME_STAMP);
      ...
      // Erase transaction is directly submitted to TSU.
