...
namespace SSD_Components
{
	...
	GC_and_WL_Unit_Page_Level::GC_and_WL_Unit_Page_Level(const sim_object_id_type& id,
		...
		: GC_and_WL_Unit_Base(id, address_mapping_unit, block_manager, tsu, ecc, flash_controller, block_selection_policy, gc_threshold, preemptible_gc_enabled, gc_hard_threshold,
    ...
    LOG();
		...
		if (free_block_pool_size < block_pool_gc_threshold) {
			...
			if (block_manager->Can_execute_gc_wl(gc_candidate_address)) {
						...
						if (block_manager->Is_page_valid(block, pageID)) {
							...
							if (use_copyback) {
								gc_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
									NO_LPA, address_mapping_unit->Convert_address_to_ppa(gc_candidate_address), DEFAULT_LPSA, nullptr, 0, nullptr, 0, INVALID_TIME_STAMP);
                ...
                ecc->submit_write_transactions(gc_write) ;
							} else {
								gc_read = new NVM_Transaction_Flash_RD(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
									NO_LPA, address_mapping_unit->Convert_address_to_ppa(gc_candidate_address), DEFAULT_LPSA, gc_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
								gc_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
									NO_LPA, NO_PPA, DEFAULT_LPSA, gc_candidate_address, NULL, 0, gc_read, 0, INVALID_TIME_STAMP);
