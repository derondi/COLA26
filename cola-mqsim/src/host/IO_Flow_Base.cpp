...
IO_Flow_Base::IO_Flow_Base(const sim_object_id_type &name, uint16_t flow_id, LHA_type start_lsa_on_device, LHA_type end_lsa_on_device, uint16_t io_queue_id,
						   ...
						   bool enabled_logging, sim_time_type logging_period, std::string logging_file_path) : MQSimEngine::Sim_Object(name), flow_id(flow_id), start_lsa_on_device(start_lsa_on_device), end_lsa_on_device(end_lsa_on_device), io_queue_id(io_queue_id),
																												...
																												STAT_P999_device_response_time_read(0),
																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	void IO_Flow_Base::SATA_consume_io_request(Host_IO_Request* request)
	{
		...
		if (request->Type == Host_IO_Request_Type::READ) {
			...
			sim_time_type read_time = device_response_time;
			if (STAT_serviced_read_request_count > 0){
				size_t target_size = std::max(static_cast<size_t>(1), static_cast<size_t>(STAT_serviced_read_request_count * 0.001));
    
				if(STAT_P999_device_response_time_read.size() < target_size){
					STAT_P999_device_response_time_read.push_back(read_time);
					std::push_heap(STAT_P999_device_response_time_read.begin(),
					STAT_P999_device_response_time_read.end(),std::greater<double>());
				}

				if(read_time > STAT_P999_device_response_time_read.front()){

					std::pop_heap(STAT_P999_device_response_time_read.begin(),
					STAT_P999_device_response_time_read.end(), std::greater<double>());
					STAT_P999_device_response_time_read.pop_back();
	...
	double IO_Flow_Base::Get_P999_device_response_time_read()
	{
		if (STAT_P999_device_response_time_read.size() == 0) {
			return 0;
		}
		double sum = 0;
		for (double time : STAT_P999_device_response_time_read){
			sum += time;
		}
        return ((double)sum / (static_cast<size_t>(STAT_serviced_read_request_count * 0.001)) / SIM_TIME_TO_MICROSECONDS_COEFF);
		//return ((double)STAT_sum_device_response_time / STAT_serviced_request_count / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_device_response_time()
	{
	...
	double IO_Flow_Base::Get_min_device_response_time()
	{
		return ((double)STAT_min_device_response_time / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_max_device_response_time()
	{
		return ((double)STAT_max_device_response_time / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_end_to_end_request_delay()
	{
		if (STAT_serviced_request_count == 0) {
			return 0;
		}

		return ((double)STAT_sum_request_delay / STAT_serviced_request_count / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_min_end_to_end_request_delay()
	{
		return ((double)STAT_min_request_delay / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_max_end_to_end_request_delay()
	{
		return (uint32_t)(STAT_max_request_delay / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_device_response_time_short_term()
	{
		if (STAT_serviced_request_count_short_term == 0) {
			return 0;
		}

		return ((double)STAT_sum_device_response_time_short_term / STAT_serviced_request_count_short_term / SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	double IO_Flow_Base::Get_end_to_end_request_delay_short_term()
	{
		if (STAT_serviced_request_count == 0) {
			return 0;
		}

		return ((double)STAT_sum_request_delay_short_term / STAT_serviced_request_count_short_term / SIM_TIME_TO_MICROSECONDS_COEFF);
	}
    ...
    if (STAT_serviced_read_request_count == 0) {
      ...
      val = std::to_string((double) STAT_sum_device_response_time_read / (double)STAT_serviced_read_request_count / SIM_TIME_TO_MICROSECONDS_COEFF);
    }
		xmlwriter.Write_attribute_string(attr, val);

	attr = "Device_P999_Response_Time_Read";
	val = std::to_string(Get_P999_device_response_time_read());
	xmlwriter.Write_attribute_string(attr, val);
    ...
    if (STAT_serviced_write_request_count == 0) {
      ...
      val = std::to_string((double) STAT_sum_device_response_time_write / (double)STAT_serviced_write_request_count / SIM_TIME_TO_MICROSECONDS_COEFF);
    }
		xmlwriter.Write_attribute_string(attr, val);
