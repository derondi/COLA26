...
void IO_Flow_Trace_Based::Start_simulation()
{
...
#ifdef ALIYUN_TRACE
    last_request_arrival_time *= 1000; // microseconds to nanoseconds
#endif
  ...
  sim_time_type timestamp = std::strtoll(current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10);
#ifdef ALIYUN_TRACE
  timestamp *= 1000;
#endif
  Simulator->set_start_time(timestamp);
	Simulator->Register_sim_event(timestamp, this);
}
...
void IO_Flow_Trace_Based::Execute_simulator_event(MQSimEngine::Sim_Event *ev)
{
  LOG("object=%s", ev->Target_sim_object->ID().c_str());

	Host_IO_Request *request = Generate_next_request();
  LOG("%lu %lu %lu %s", request->Arrival_time, request->LBA_count, request->Start_LBA,
    ...
    timestamp *= 1000;
		...
		Simulator->Register_sim_event(time_offset + timestamp, this);
	}
}
...
#ifdef ALIYUN_TRACE 
    last_request_arrival_time *= 1000;
#endif
