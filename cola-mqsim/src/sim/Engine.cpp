...
namespace MQSimEngine
{
    ...
    is_preconditioning = false;
      ...
      // LOG("nevent=%d", _EventList->Count);
        ...
        fprintf(stderr, "\n");
        #endif
        LOG("fireTime=%lu, nevents=%d", ev->Fire_time, _EventList->Count);
				if(!ev->Ignore) {
	...
	{
    LOG("id=%s, fireTime=%lu", targetObject->ID().c_str(), fireTime);
  ...
  void Engine::set_start_time(sim_time_type start_time) 
  {
    this->start_time = start_time;
  }
  
  sim_time_type Engine::get_start_time() 
  {
    return start_time;
  }
}