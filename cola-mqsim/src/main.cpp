...
// For debug
bool is_preconditioning = true;

void command_line_args(char* argv[], string& input_file_path, string& workload_file_path, string& ecc_config_file_path)
{
    ...
    char ecc_path_switch[] = "-e";
 		if (arg.compare(0, strlen(ecc_path_switch), ecc_path_switch) == 0) {
			ecc_config_file_path.assign(argv[++arg_cntr]);
      LOG("%s", ecc_config_file_path.c_str());
			continue;
		}
	}
}
...
void read_ecc_config_parameters(const string &ecc_config_file_path)
{
  ifstream ecc_config_file; 
  ecc_config_file.open(ecc_config_file_path);
  auto ecc_param = ECC_Parameter_Set();
  
	if (!ecc_config_file) {
		  PRINT_ERROR("The specified ECC configuration file (" << ecc_config_file_path << ") does not exist.");
	} else {
		  //Read input workload parameters
		  string line((std::istreambuf_iterator<char>(ecc_config_file)),
			std::istreambuf_iterator<char>());
		  ecc_config_file >> line;
			rapidxml::xml_document<> doc;    // character type defaults to char
			char* temp_string = new char[line.length() + 1];
			strcpy(temp_string, line.c_str());
			doc.parse<0>(temp_string);
			rapidxml::xml_node<> *ecc_config = doc.first_node("ecc_parameter_set");
			if (ecc_config != NULL) {
				ecc_param.XML_deserialize(ecc_config);
			} else {
				PRINT_MESSAGE("Error in the ECC configuration file!")
				PRINT_MESSAGE("Using MQSim's default configuration.")
			}
		}

	ecc_config_file.close();
}
...
void print_help()
{
	cout << "MQSim - SSD simulator with both NVMe and SATA host interface behavior, see ReadMe.md for details" << endl <<
		...
		"./MQSim [-i path/to/config/file] [-w path/to/workload/file] [-e path/to/ecc/file]" << endl;
}

int main(int argc, char* argv[])
{
	string ssd_config_file_path, workload_defs_file_path, ecc_config_file_path;
	if (argc != 7) {
		// MQSim expects 2 arguments: 1) the path to the SSD configuration definition file, 2) the path to the workload definition file, and 3) the path to the ecc config file
		print_help();
		return 1;
	}

	command_line_args(argv, ssd_config_file_path, workload_defs_file_path, ecc_config_file_path);
  ...
  read_ecc_config_parameters(ecc_config_file_path);
    ...
    // Convert to time_t to extract date and time
    std::time_t timestamp = std::chrono::system_clock::to_time_t(now);
    // Use localtime to convert time_t to tm struct
    std::tm tm = *std::localtime(&timestamp);
    // Use stringstream to format the timestamp
    std::stringstream ss;
