...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
{
		...
		case SSD_Components::ONFI_Protocol::NVDDR2:
		{
			...
			for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++)
			{
				...
				channels[channel_cntr] = new SSD_Components::ONFI_Channel_NVDDR2(channel_cntr, parameters->Chip_No_Per_Channel,
																				 ...
																				 ((double)1000 / parameters->Channel_Transfer_Rate) * 2, ((double)1000 / parameters->Channel_Transfer_Rate) * 2);
    ...
    //Steps 3.5: create ECCEngine
    device->ECC = new SSD_Components::ECCEngine(device->ID() + ".ECCEngine", parameters->Flash_Channel_Count, (SSD_Components::NVM_PHY_ONFI*)this->PHY, NULL, 
	              ECC_Parameter_Set::ldpc_encoder_param_8kb, ECC_Parameter_Set::ldpc_hard_decoder_param_8kb, ECC_Parameter_Set::ldpc_soft_decoder_param_8kb,
	              ECC_Parameter_Set::ldpc_encoder_param_4kb, ECC_Parameter_Set::ldpc_hard_decoder_param_4kb, ECC_Parameter_Set::ldpc_soft_decoder_param_4kb,
                  ECC_Parameter_Set::ldpc_encoder_param_2kb, ECC_Parameter_Set::ldpc_hard_decoder_param_2kb, ECC_Parameter_Set::ldpc_soft_decoder_param_2kb,
				  ECC_Parameter_Set::ldpc_encoder_param_1kb, ECC_Parameter_Set::ldpc_hard_decoder_param_1kb, ECC_Parameter_Set::ldpc_soft_decoder_param_1kb);
    Simulator->AddObject(device->ECC);
    ...
    device->ECC->SetTSU(tsu);
		...
		case SSD_Components::Flash_Address_Mapping_Type::PAGE_LEVEL:
			amu = new SSD_Components::Address_Mapping_Unit_Page_Level(ftl->ID() + ".AddressMappingUnit", ftl, (SSD_Components::NVM_PHY_ONFI *)device->PHY, device->ECC,
		...
		case SSD_Components::Flash_Address_Mapping_Type::HYBRID:
			amu = new SSD_Components::Address_Mapping_Unit_Hybrid(ftl->ID() + ".AddressMappingUnit", ftl, device->ECC, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
		...
		gcwl = new SSD_Components::GC_and_WL_Unit_Page_Level(ftl->ID() + ".GCandWLUnit", amu, fbm, tsu, device->ECC, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
		...
		case SSD_Components::Caching_Mechanism::SIMPLE:
			dcm = new SSD_Components::Data_Cache_Manager_Flash_Simple(device->ID() + ".DataCache", NULL, ftl, device->ECC, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
		...
		case SSD_Components::Caching_Mechanism::ADVANCED:
			dcm = new SSD_Components::Data_Cache_Manager_Flash_Advanced(device->ID() + ".DataCache", NULL, ftl, device->ECC, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
																		parameters->Data_Cache_Capacity, parameters->Data_Cache_DRAM_Row_Size, parameters->Data_Cache_DRAM_Data_Rate,
																		parameters->Data_Cache_DRAM_Data_Busrt_Size, parameters->Data_Cache_DRAM_tRCD, parameters->Data_Cache_DRAM_tCL, parameters->Data_Cache_DRAM_tRP,
...
void SSD_Device::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter &xmlwriter)
{
  ...
  this->ECC->Report_results_in_XML(ID(), xmlwriter);
