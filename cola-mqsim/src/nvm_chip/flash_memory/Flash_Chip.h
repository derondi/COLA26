...
namespace NVM
{
	namespace FlashMemory
	{
        ...
        LOG();
			...
			void EndCMDXfer(Flash_Command* command, int factor = 1)//End transferring write data to the Flash chip
			{
        LOG();
				...
				start_command_execution(command, factor);
        ...
        LOG();
			...
			void EndDataOutXfer(const Flash_Command* command)
			{
        LOG();
			...
			sim_time_type Get_command_execution_latency(command_code_type CMDCode, flash_page_ID_type pageID, int factor = 1)
			{
        LOG();
				...
				switch (CMDCode)
				{
					...
					case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
						return _readLatency[latencyType] / factor + _RBSignalDelayRead;
			...
			ECCMetadata GetECCMetadata(flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id, flash_page_ID_type page_id)
      {
        const Block *block = Dies[die_id]->Planes[plane_id]->Blocks[block_id];
        const Page &page = block->Pages[page_id];

        
        return ECCMetadata{block->EraseCount, page.RetentionTime, page.Reads, page.CodewordType, page.CodewordNumber};
      }
		private:
			Flash_Technology_Type flash_technology;
      ...
      // Factor for hard decode read latency, if this transaction is hard decoding, tR_hard = tR / 3
			void start_command_execution(Flash_Command* command, int factor = 1);
