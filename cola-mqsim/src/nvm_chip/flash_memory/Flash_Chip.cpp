...
namespace NVM
{
	namespace FlashMemory
	{
      ...
      LOG();
		...
		{
      LOG("object=%s", ev->Target_sim_object->ID().c_str());
      ...
      LOG();
		...
		void Flash_Chip::start_command_execution(Flash_Command* command, int factor)
		{
      LOG();
			...
			targetDie->Expected_finish_time = Simulator->Time() + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID, factor);
      ...
      LOG();
			...
			switch (command->CommandCode)
			{
				...
				case CMD_READ_PAGE_COPYBACK_MULTIPLANE: {
					...
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						Plane *targetPlane = targetDie->Planes[command->Address[planeCntr].PlaneID];
            targetPlane->Read_count++;
            
            Page &targetPage = targetPlane->Blocks[command->Address[planeCntr].BlockID]->Pages[command->Address[planeCntr].PageID];
						targetPage.Read_metadata(command->Meta_data[planeCntr]);
            
            LOG("channel: %lu, chip: %lu, die: %lu, plane: %lu, block: %lu, page: %lu", 
              command->Address[planeCntr].ChannelID, command->Address[planeCntr].ChipID, command->Address[planeCntr].DieID, command->Address[planeCntr].PlaneID,
              command->Address[planeCntr].PlaneID, command->Address[planeCntr].BlockID, command->Address[planeCntr].PageID
            );

            // Increase target page's Reads and set new retention time
            LOG("before Reads = %lu", targetPage.Reads);
            targetPage.Reads++;
            LOG("after Reads = %lu", targetPage.Reads);


            if (targetPage.LastWrittenTime == 0) 
            {
              // This means this page is never programmed.
              // The unit is days, ns / 1000 000 000 -> s / 3600 -> h / 24 -> day
              targetPage.RetentionTime = (Simulator->Time() - Simulator->get_start_time()) / 1000000000.0 / 3600.0 / 24.0 + ECC_Parameter_Set::ssd_init_retention_days;
            } 
            else 
            {
              // This means this page has been programed
              targetPage.RetentionTime = (Simulator->Time() - targetPage.LastWrittenTime) / 1000000000.0 / 3600.0 / 24.0;
            }
					}
					break;
        }
				...
				case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
					...
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						STAT_progamCount++;
						Plane *targetPlane = targetDie->Planes[command->Address[planeCntr].PlaneID];
						targetPlane->Progam_count++;
						Block *targetBlock = targetPlane->Blocks[command->Address[planeCntr].BlockID];
						//targetDie->Planes[command->Address[planeCntr].PlaneID]->Progam_count++;
						Page &targetPage = targetBlock->Pages[command->Address[planeCntr].PageID];
                        targetPage.Write_metadata(command->Meta_data[planeCntr]);
                        ...
                        targetPage.LastWrittenTime = Simulator->Time();
            ...
            targetBlock->EraseCount++;
              ...
              // Clear target page retention time, read times and last wriiten time
              targetBlock->Pages[i].RetentionTime = 0;
              targetBlock->Pages[i].Reads = 0;
              targetBlock->Pages[i].LastWrittenTime = Simulator->Time();
						}
					}
					break;
				}
